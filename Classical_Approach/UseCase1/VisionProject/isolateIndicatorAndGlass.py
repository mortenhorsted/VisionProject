...
def isolateIndicatorAndGlass(inputImage):
    ...
    x_inner, y_inner, r_inner = detectCircle(tempImage, 120, 220)
    x_outer, y_outer, r_outer = detectCircle(tempImage, 320, 450)
    #cv2.imwrite("Images/RPI_Images/CirclesMarked.jpg", tempImage)
    ...
    #cv2.imwrite("Images/RPI_Images/mask1_indicator.jpg", mask1)
    #cv2.imwrite("Images/RPI_Images/invertedIndicatorMask.jpg", invertedIndicatorMask)
    mask2 = np.zeros_like(inputImage)
    fullMaskGlass = cv2.circle(mask2, (x_outer, y_outer), r_outer, (255, 255, 255), -1)
    #cv2.imwrite("Images/RPI_Images/fullMaskGlass.jpg", fullMaskGlass)
    ...
    #cv2.imwrite("Images/RPI_Images/halvedMaskIndicator.jpg", halvedMaskIndicator)
    #cv2.imshow("TEMP", halvedMaskIndicator)
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()
